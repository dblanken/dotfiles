...
dblanken_commit_hash() {
  local commit_hash="$(git log -1 --pretty=format:%h 2>/dev/null)"

  if [[ -n "$commit_hash" ]]; then
    echo "[%F{yellow}$commit_hash%f] "
  fi
}
...
RPROMPT_BASE="\$(dblanken_ruby)\$(dblanken_commit_hash)\${vcs_info_msg_0_}%F{blue}%~%f"
setopt PROMPT_SUBST
